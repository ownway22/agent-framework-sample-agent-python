...
class GenericAgentHost:
    ...
    def start_server(self, auth_configuration: AgentAuthConfiguration | None = None):
        ...
        async def index(_req: Request) -> Response:
            auth_mode = "Enabled" if auth_configuration else "Anonymous"
            html = f"""<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>{self.agent_class.__name__}</title>
    <style>
        :root {{
            color-scheme: light;
            --bg: #f5f1e8;
        ...
        * {{ box-sizing: border-box; }}
        body {{
            ...
            background: radial-gradient(circle at top, #fffaf0 0%, var(--bg) 52%, #ebe5d8 100%);
            color: var(--ink);
        }}
        main {{
            max-width: 780px;
...
<body>
    <main>
        <section class="card">
            <span class="eyebrow">Agent Server Ready</span>
            <h1>{self.agent_class.__name__}</h1>
            <p>This local server is running correctly. The browser root page is informational only; the agent itself listens on the API endpoints below.</p>
            <div class="grid">
                <div class="tile">
                ...
                <div class="tile">
                    <strong>Message Endpoint</strong>
                    <div><code>/api/messages</code></div>
                </div>
                <div class="tile">
            ...
            <p>For local testing with Agents Playground, point it to <code>http://localhost:{port}/api/messages</code>.</p>
            <ul>
                <li>Health check: <a href="/api/health">/api/health</a></li>
                <li>API endpoint: <code>POST /api/messages</code></li>
                <li>If you want MCP tools to load, set <code>BEARER_TOKEN</code> or enable agentic auth in <code>.env</code>.</li>
            </ul>
        </section>
    </main>
</body>
